...
async function runMigrations() {
  ...
  // Visitor location from Vercel's edge geo headers (city level) plus the
  // visitor's IP, so the owner can look up which practice or business a
  // visit came from. Added after launch, so ALTER rather than relying on
  // CREATE TABLE.
